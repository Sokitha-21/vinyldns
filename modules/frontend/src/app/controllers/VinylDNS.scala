    ...
    details.value
      .map {
        ...
        case Left(err) =>
          ...
          Redirect("/login?error=login_failed").withNewSession
      }
      .unsafeToFuture()
  }

  def login(): Action[AnyContent] = Action { implicit request =>
    if (request.contentType.exists(_.contains("application/json"))) {
      // React SPA sends JSON — return JSON so fetch() can parse the response.
      val (username, password) = request.body.asJson match {
        case Some(json) =>
          ((json \ "username").asOpt[String].getOrElse(""),
           (json \ "password").asOpt[String].getOrElse(""))
        case None => ("", "")
      }
      authenticator.authenticate(username, password) match {
        case Left(error: UserDoesNotExistException) =>
          ...
          Unauthorized(Json.obj("ok" -> false, "error" -> "Authentication failed, please try again"))
        case Left(error: LdapException) =>
          logger.error(s"Unexpected LDAP error during login for [$username]", error)
          Unauthorized(Json.obj("ok" -> false, "error" -> "Authentication failed, please contact your VinylDNS administrators"))
      ...
      // Legacy form-encoded POST — keep redirect behaviour for non-SPA clients.
      val userForm = Form(tuple("username" -> text, "password" -> text))
