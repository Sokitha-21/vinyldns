  ...
  return (
    <div className="d-flex flex-column gap-1">
      {visible.map((val, i) => (
        <span
          key={i}
          className="vds-record-data-chip vds-record-data-chip--wrap"
        >
          <i
            className="bi bi-dot"
            style={{ fontSize: "0.65rem", marginRight: 2, opacity: 0.5 }}
          />
          {val}
        </span>
      ))}
...
function formatRecordData(r: RecordSet): React.ReactNode {
  ...
  if (r.type === "NS" || r.type === "PTR") {
    ...
    const all = r.records.map(
      (d: RecordData) => (d as Record<string, string>)[key] ?? "",
    );
      ...
      <div className="d-flex flex-column gap-1">
        <span className="vds-record-data-chip vds-record-data-chip--wrap">
          <i
            ...
            style={{ fontSize: "0.65rem", marginRight: 3, opacity: 0.6 }}
          />
          {d.mname}
        </span>
        <span className="vds-record-data-chip vds-record-data-chip--wrap">
  ...
  const items = r.records.slice(0, 3).map((d: RecordData, i: number) => {
    ...
    switch (r.type) {
      ...
      case "CNAME":
        ...
        break;
      case "MX":
        text = `${d.preference ?? 10} ${d.exchange ?? ""}`;
        break;
      case "TXT":
      case "SPF":
        text = `"${d.text ?? ""}"`;
        break;
      case "SRV":
        text = `${d.priority} ${d.weight} ${d.port} ${d.target}`;
        break;
      case "CAA":
        text = `${d.flags} ${d.tag} "${d.value}"`;
        break;
      case "SSHFP":
        text = `alg ${d.algorithm} fp ${String(d.fingerprint ?? "").slice(0, 12)}…`;
        break;
      case "DS":
        text = `tag ${d.keytag} alg ${d.algorithm} digest ${d.digesttype}`;
        break;
      case "NAPTR":
        text = `${d.order} ${d.preference} ${d.flags} ${d.service}`;
        break;
      default:
        text = JSON.stringify(d).slice(0, 40);
    }
    return (
    ...
    <div className="d-flex flex-wrap gap-1 align-items-center">
      ...
      {r.records.length > 3 && (
        <span
          ...
          title={r.records
            .slice(3)
            .map((d: RecordData) => JSON.stringify(d))
            .join("\n")}
        >
          +{r.records.length - 3} more
        </span>
      )}
    </div>
  );
...
const OWNERSHIP_STATUS_META: Record<
  string,
  { cls: string; icon: string; label: string }
> = {
  AutoApproved: {
    cls: "vds-ownership-badge--approved",
  ...
  ManuallyRejected: {
    cls: "vds-ownership-badge--rejected",
    icon: "bi-x-circle-fill",
    label: "Rejected",
  },
  Requested: {
    cls: "vds-ownership-badge--requested",
    icon: "bi-arrow-right-circle-fill",
    label: "Requested",
  },
  PendingReview: {
    cls: "vds-ownership-badge--pending",
    icon: "bi-hourglass-split",
    label: "Pending Review",
  },
  Cancelled: {
    cls: "vds-ownership-badge--cancelled",
    icon: "bi-ban",
    label: "Cancelled",
  },
};
...
function OwnershipStatusBadge({ status }: { status?: string }) {
  if (!status || status === "None") {
    return (
      <span className="vds-ownership-badge vds-ownership-badge--none">—</span>
    );
  }
  const meta = OWNERSHIP_STATUS_META[status];
  if (!meta)
    return (
      <span className="vds-ownership-badge vds-ownership-badge--none">
        {status}
      </span>
    );
  return (
...
function resolveOwnershipCapability(
  ...
  const hasNonOwnerGroup = userGroupIds.some(
    (gid) => gid !== record.ownerGroupId,
  );
...
function ZoneTypePill({ isSharedZone }: { isSharedZone: boolean }) {
  return (
    <span
      className={`vds-records-zone-pill ${isSharedZone ? "vds-records-zone-pill--shared" : "vds-records-zone-pill--private"}`}
    >
      <i
        className={`bi ${isSharedZone ? "bi-share-fill" : "bi-lock-fill"} me-1`}
...
function OwnershipActionsDropdown({
  canCancel,
  isPendingOnly = false,
  onApprove,
  onReject,
  onCancel,
}: OwnershipActionsDropdownProps) {
  ...
  return (
    <div className="vds-ownership-dropdown" ref={ref}>
      <button
        ...
        title={
          isPendingOnly
            ? "Your request is pending — click to cancel"
            : "Ownership actions"
        }
      >
        <i
          className={`bi ${isPendingOnly ? "bi-hourglass-split" : "bi-shield-fill-check"}`}
        />
        <i
          className={`bi bi-chevron-${open ? "up" : "down"} vds-ownership-dropdown__caret`}
        />
      </button>
      ...
      {open && (
        <div className="vds-ownership-dropdown__menu">
          {/* Menu header */}
          <div
            className={`vds-ownership-dropdown__header ${isPendingOnly ? "vds-ownership-dropdown__header--pending" : ""}`}
          >
            <i
              className={`bi ${isPendingOnly ? "bi-hourglass-split" : "bi-shield-fill-check"} me-2`}
          ...
          {canCancel && onCancel && (
            <>
              {!isPendingOnly && (
                <div className="vds-ownership-dropdown__divider" />
              )}
              <button
...
export function RecordsTable({
  ...
  if (!records.length) {
    return (
      <div className="vds-empty-state">
        <i
          className="bi bi-file-earmark-text fs-1 mb-2"
          style={{ opacity: 0.35 }}
        />
        <p className="mb-0 fw-semibold">No records found</p>
        <small className="text-muted">
          Try adjusting the filter or add a new record.
        </small>
      </div>
    );
  }
  ...
  const hasActions = !!(
    onEdit ||
    onDelete ||
    (isSharedZone && (onRequestOwnership || onApproveOwnership))
  );
              ...
              <>
                <th>
                  <span className="vds-th-with-icon">
                    <i
                      className="bi bi-people-fill me-1 text-primary"
                      style={{ fontSize: "0.75rem" }}
                    />
                    Owner Group Name
                  </span>
                ...
                <th>
                  <span className="vds-th-with-icon">
                    <i
                      className="bi bi-arrow-left-right me-1 text-warning"
                      style={{ fontSize: "0.75rem" }}
                    />
        ...
        <tbody>
          {records.map((rec) => {
            const capability: OwnershipCapability | null = isSharedZone
              ? resolveOwnershipCapability(rec, {
                  isSuper,
                  isSupport,
                  isZoneAdmin,
                  userGroupIds,
            ...
            return (
              <tr key={rec.id}>
                ...
                <td>
                  <span className="vds-record-type-badge">{rec.type}</span>
                </td>
                ...
                <td>
                  <span
                    className={`vds-zone-status-badge ${statusClass(rec.status)}`}
                  >
                    {rec.status}
                  </span>
                </td>
